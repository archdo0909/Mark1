...
int num_count = 0;
int tri_count = 0;
double View_from[3] = { 0.0, -100.0, -50.0 };
...
#define node_Num_n /*face_info[0].node_Num_h*/ 21
	...
	/*natural_length_x = fabs((double)(node_point[0][0].pos.x[0] - node_point[0][1].pos.x[0]) / (double)(face_info[0].node_Num_w - 1));
	natural_length_y = fabs((double)(node_point[0][1].pos.x[1] - node_point[0][2].pos.x[1]) / (double)(face_info[0].node_Num_h - 1));*/
	...
	for (i = 0; i < node_Num_m; i++){
		for (j = 0; j < node_Num_n; j++){
			...
			/*	if (i == 0 || j == 0 || i == node_Num_m - 1 || j == node_Num_n - 1){
				...
				node_surface[i][j][k].pos.x[0] = (double)(i * natural_length_x);
				node_surface[i][j][k].pos.x[1] = (double)(j * natural_length_y);
...
void display(void)
{
	...
	for (i = 0; i < num_count; i++){
		...
		glTranslated((GLdouble)node_surface2[i].pos.x[2], (GLdouble)node_surface2[i].pos.x[0], (GLdouble)node_surface2[i].pos.x[1]);
		/*printf("node_surface2[0].pos.x[0] = %lf\n", node_surface2[0].pos.x[0]);
		printf("node_surface2[0].pos.x[1] = %lf\n", node_surface2[0].pos.x[1]);
		printf("node_surface2[0].pos.x[2] = %lf\n", node_surface2[0].pos.x[2]);*/
		/*glTranslated(0, 0, 0);*/
